...
namespace config_loader
{
    ...
    bool load_accel_calibration(const std::string &config_path,
                                ...
                                std::array<float, 9> &accel_matrix)
    {
        std::ifstream file(config_path);
        if (!file.is_open())
        {
        ...
        while (std::getline(file, line))
        {
            ...
            if (line.find("accel_bias:") != std::string::npos)
            {
                ...
                if (parse_float_array(line, values) && values.size() == 3)
                {
                    for (size_t i = 0; i < 3; ++i)
                    {
                        accel_bias[i] = values[i];
                    }
                    found_bias = true;
                }
            }

            // Look for accel_matrix (may span multiple lines)
                               ...
                               std::array<float, 9> &gyro_matrix)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            // Look for gyro_bias
            if (line.find("gyro_bias:") != std::string::npos)
            {
                std::vector<float> values;
                if (parse_float_array(line, values) && values.size() == 3)
                {
            ...
            // Look for gyro_matrix (may span multiple lines)
            if (line.find("gyro_matrix:") != std::string::npos)
            {
                matrix_content = line;
